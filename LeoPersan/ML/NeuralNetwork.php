...
class NeuralNetwork
{
    ...
    {
        $model = json_decode(file_get_contents($string), true);
        foreach ($model as $i => $layer) {
            $numInputs = count($layer[0]);
            $numNeurons = count($layer);
            $activationFunction = new ActivationFunction();
            $this->addLayer(new Layer($numInputs, $numNeurons, $activationFunction));
            foreach ($layer as $j => $weights) {
                $neuron = $this->layers[$i]->getNeurons()[$j];
